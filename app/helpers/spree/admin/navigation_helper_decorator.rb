Spree::Admin::NavigationHelper.module_eval do
  ...
  def dropdow_tab(*args)
    ...
    link_down = link_to("<i class='icon-caret-left'></i> #{t(options[:label].to_sym, :default => options[:label]).titleize}".html_safe, "javascript: void(0)", :class => "dropdown-toggle", :id => "a-cms-list")
    down = content_tag(:div, child.html_safe, :class => "dropdown", :id => "sub-cms-list")
    content_tag('li', link_down + down, :class => "#{options[:class]} tab-with-icon", :id => "cms-list")
  end
end