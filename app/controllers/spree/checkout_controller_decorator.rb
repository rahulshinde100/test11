...
module Spree
  CheckoutController.class_eval do
    ...
    def registration
      @user = Spree::User.new
      session["spree_user_return_to"] = "cart".to_sym
    end
    ...
    def paypal_payment
      ...
      opts.merge!({:no_shipping => false})

      opts.delete(:callback_url)
      opts.delete(:callback_timeout)
      opts.delete(:callback_version)
      opts.delete(:shipping_options)
      opts.delete(:max_amount)
      ...
      if @ppx_details.success?
        ...
        unless payment_method.preferred_no_shipping
          ...
          order_ship_address = Spree::Address.new :firstname  => @ppx_details.params["first_name"],
                                                  ...
                                                  :phone      => @ppx_details.params["phone"] || "#{@order.ship_address.phone}"
          ...
          if state
            ...
            order_ship_address.state_name = (ship_address["state"] || "Singapore").capitalize
          end
    ...
    def load_order
      if (!params[:order_id].blank? and params[:target] == "ios")
        ...
        session[:access_token] = Spree::TokenizedPermission.find_by_permissable_id(order_id).token
      end

      @order = current_order || get_order_from_session()
    ...
    def before_address
      unless spree_current_user.nil?
        if @order.bill_address.nil?
          @order.bill_address = spree_current_user.orders.complete.blank? ? Address.default : spree_current_user.orders.complete.last.bill_address
          @order.ship_address = spree_current_user.orders.complete.blank? ? Address.default : spree_current_user.orders.complete.last.ship_address
        else
    ...
    def redirect_to_paypal_express_form_if_needed
      ...
      if @order.update_attributes(object_params)
        if params[:order][:coupon_code] and !params[:order][:coupon_code].blank? and @order.coupon_code.present?
          ...
          #ambassador code here
#          ambassador_promo = Spree::Promotion.where(:name => "ambassador", :code => "zxcvbnm")
#          refer = Mbsy::Ambassador.find(:email => "#{spree_current_user.email}")
#          if refer["ambassador"]["balance_money"].to_i < 10 && ambassador_promo.code == params[:order][:coupon_code]
#            flash[:notice] = "Invalid coupon code"
#            redirect_to checkout_state_path(@order.state)
#            return
#          end
          #####################
          event_name = "spree.checkout.coupon_code_added"
          if promo = Spree::Promotion.where(:code => @order.coupon_code, :event_name => event_name).first
        ...
        elsif params[:store_return_payment] == "true"
          flash[:notice] = "Store credit redeemed successfully"
          redirect_to checkout_state_path(@order.state)
          return
        elsif params[:coupon_return_payment] == "true"
          flash[:error] = t(:promotion_not_found)
          redirect_to checkout_state_path(@order.state)
          return
        end
    ...
    def order_opts(order, payment_method_id, stage)
      ...
      ship_amount = (order.ship_total*100).to_i
      free_shipping_flag = false
      credits = order.adjustments.eligible.map do |credit|
        if (credit.is_free_shipping?(order) || (free_shipping_flag == true and credit.label == "Shipping"))
