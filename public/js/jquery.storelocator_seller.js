...
if( address.substr(address.length - 1) == ',')
  address = address.slice(0,-1);
                ...
                if(settings.maxDistance === true && firstRun !== true){
                  if(distance < maxDistance){
                    locationset[i] = [distance, name, lat, lng, address, address2, city, state, postal, country, phone, email, web, hours1, hours2, hours3, category, loc_id , locname];
                  }
                  else{
                    return;
                  }
                  ...
                  locationset[i] = [distance, name, lat, lng, address, address2, city, postal, state, country, phone, email, web, hours1, hours2, hours3, category, loc_id, locname];
                }
              ...
              $(data).find('marker').each(function(){
                ...
                if(settings.maxDistance === true && firstRun !== true){ 
                  if(distance < maxDistance){
                    locationset[i] = [distance, name, lat, lng, address, address2, city, postal, country, phone, email, web, hours1, hours2, hours3, category];
                  }
                  else{
                    return;
                  }
          ...
          $(function(){ 
              ...
              var storeDistance, storeName, storeAddress1, storeAddress2, storeCity, storeState, storeCountry, storeZip, storePhone, storeEmail, storeWeb, storeHours1, storeHours2, storeHours3, storeDescription, storeCat, storeID, locName;
              ...
              function create_location_variables(loopcount){
                ...
                storeID = locationset[loopcount][17]; 
                locName = locationset[loopcount][18]; 
              }
              ...
              if(settings.originMarker === true && settings.fullMapStart === false){
                var originPinShadow = new google.maps.MarkerImage("https://chart.apis.google.com/chart?chst=d_map_pin_shadow",
              ...
              $(document).on('click.'+prefix, '#' + settings.listDiv + ' li', function(){
                ...
                if($('#' + settings.listDiv + ' li[data-markerid=' + markerId +']').children().hasClass("store-pin"))
                    {
                        
                        $('#map-box').css('visibility', 'visible');
                        $('#map-box').fadeIn("slow");
                        $('#map-box').css("position", "relative");  
                    }
                    else{
                ...
                //$('body').append('<div class="gmap-overlay"></div>');

                map.panTo(selectedMarker.getPosition());
                var listLoc = "left";
                if(settings.bounceMarker === true){
                  selectedMarker.setAnimation(google.maps.Animation.BOUNCE);
                  setTimeout(function() { selectedMarker.setAnimation(null); create_infowindow(selectedMarker, listLoc); 
                    }, 700);
                }
                else{
              ...
              function createMarker(point, name, address, letter, type){
                ...
                var pinImage = new google.maps.MarkerImage("//chart.apis.google.com/chart?chst=d_map_pin_letter&chld=" + letter + "|" + settings.pinColor + "|" + settings.pinTextColor,
